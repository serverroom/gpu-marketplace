...
// WSLHost is the machine a Windows host rents out: a WSL 2 distribution the
// agent owns, reached through wsl.exe. The container runtime, the fence and the
// encrypted volumes run inside it exactly as on a Linux host; only the way a
// command reaches them differs. Every path is a Linux path in the distribution.
type WSLHost struct {
	Exec WSLExec
...
func (w WSLHost) Hold() (release func()) {
	if w.Keep == nil {
		return func() {}
	}
	return w.Keep()
...
func (w WSLHost) exec(stdin []byte, argv ...string) ([]byte, []byte, int, error) {
	if w.Exec == nil {
...
// OSHost does.
func (w WSLHost) do(stdin []byte, name string, args ...string) ([]byte, []byte, error) {
...
func (w WSLHost) sh(stdin []byte, script string, args ...string) ([]byte, int, error) {
...
func (w WSLHost) Run(name string, args ...string) error {
	_, _, err := w.do(nil, name, args...)
	return err
}

func (w WSLHost) RunInput(stdin []byte, name string, args ...string) error {
	_, _, err := w.do(stdin, name, args...)
	return err
}

func (w WSLHost) Output(name string, args ...string) (string, error) {
	out, _, err := w.do(nil, name, args...)
	return string(out), err
}

func (w WSLHost) LookPath(name string) (string, error) {
...
func (w WSLHost) read(op, p, script string, args ...string) ([]byte, error) {
	p = lp(p)
...
func (w WSLHost) ReadFile(p string) ([]byte, error) {
	return w.read("open", p, `[ -e "$1" ] || exit 44; exec cat -- "$1"`)
}

func (w WSLHost) ReadTail(p string, max int64) ([]byte, error) {
	return w.read("open", p, `[ -e "$1" ] || exit 44; exec tail -c "$2" -- "$1"`, strconv.FormatInt(max, 10))
}

func (w WSLHost) WriteFile(p string, data []byte, perm os.FileMode) error {
	p = lp(p)
...
func (w WSLHost) Readlink(p string) (string, error) {
	out, err := w.read("readlink", p, `[ -L "$1" ] || exit 44; exec readlink -- "$1"`)
	return strings.TrimSuffix(string(out), "\n"), err
}

func (w WSLHost) Glob(pattern string) ([]string, error) {
...
func (w WSLHost) Exists(p string) bool {
	_, code, err := w.sh(nil, `[ -e "$1" ] || [ -L "$1" ]`, lp(p))
	return err == nil && code == 0
}

func (w WSLHost) Remove(p string) error {
	return w.Run("/bin/sh", "-c", `if [ -d "$1" ] && [ ! -L "$1" ]; then rmdir -- "$1"; else rm -f -- "$1"; fi`, "sh", lp(p))
}

func (w WSLHost) RemoveAll(p string) error { return w.Run("rm", "-rf", "--", lp(p)) }

func (w WSLHost) Rename(from, to string) error {
	return w.Run("mv", "-f", "-T", "--", lp(from), lp(to))
}

func (w WSLHost) MkdirAll(p string, perm os.FileMode) error {
	return w.Run("mkdir", "-p", "-m", strconv.FormatUint(uint64(perm.Perm()), 8), "--", lp(p))
}

func (w WSLHost) DialTCP(addr string, timeout time.Duration) error {
...
func (w WSLHost) Download(url, p string) error {
	p = lp(p)
	return w.Run("/bin/sh", "-c", `curl -fsSL --retry 3 -o "$2.part" "$1" && mv -f "$2.part" "$2"`, "sh", url, p)
}

func (w WSLHost) SHA256(p string) (string, error) {
...
func (WSLHost) Sleep(d time.Duration) { time.Sleep(d) }
